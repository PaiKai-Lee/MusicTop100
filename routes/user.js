            ...
            try {
                ...
                if (result) {
                    ...
                    if (email === result["email"]){
                        ...
                        res.send({ "status": "error", "message": "帳號已註冊" })
                    }
                } else {
                    console.log("儲存後登入")
                    let dbcol = db.db("top100");
                    let obj = { "user": user, "email": email, "user_id": user_id, "picture": picture, "third_party": true, "date": new Date() }
                    await dbcol.collection("user").insertOne(obj);
                    // 設定session //
                    req.session.valid = true;
                    req.session.user = email;
                    res.send({
